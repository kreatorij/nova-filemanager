...
trait GetFiles
{
	...
	public function getFileData($file, $id)
	{
		if (
			...
			$fileInfo = [
				...
				'size' => (0 != $file['size']) ? $file['size'] : 0,
				'size_human' => (0 != $file['size']) ? $this->formatBytes($file['size'], 0) : 0,
	...
	public function setRelativePath($folder)
	{
		...
		if ('/' != $folder) {
			$this->currentPath = $this->getAppend().'GetFiles.php/'.$publicPath;
		} else {
	...
	public function generateParent($folder)
	{
		...
		if ($paths) {
			...
			if ($folderPath == $folder || 0 === strlen($folderPath)) {
				$folderPath = '/';
			}
	...
	{
		return Str::before($pathCollection->implode('/'), $name);
	}
